...
public class DisplayDay extends JFrame implements ActionListener
{
  ...
  public DisplayDay(String dbName, String tableName, String[] columnNames)
  {
  ...
  public void actionPerformed(ActionEvent e)
  {
    ...
    String tableName = "";
    String dbName = "Golf";
    String[] dataColumnNames = {"playTimeId","ghin","lastName","firstName","teeTimeIndex","teeTimeRanking","teeTime","finalTeeTime"};
    String[] displayColumnNames = {"playTimeId","ghin", "lastName", "firstName","teeTimeIndex", "teeTime"};
    String dbQuery = "";
    int totalPlayers;
    int playersAdded;
    ...
    if (!tableName.equals(""))
    {
      try
      {
        ...
        dataList = objDb.getAllData(tableName, dataColumnNames);
        //sets all final tee times to 0
        for (int i=0; i<dataList.size(); i++)
        {
          dataList.get(i).set(7, "0");
        }
        //sorts the players
        SortPlayers sortObj = new SortPlayers(playersAdded, dataList);
        dataList = sortObj.getSortedPlayers();
        dbQuery = "DELETE FROM " + tableName;
        s.execute(dbQuery);
        //inserts the sorted values into the table
        for (int i=0; i<dataList.size(); i++)
        {
          dbQuery = "INSERT INTO " + tableName + " VALUES (?,?,?,?,?,?,?,?)";
          PreparedStatement ps = myDbConn.prepareStatement(dbQuery);
          ps.setInt(1, Integer.parseInt(dataList.get(i).get(0)));
          ps.setInt(2, Integer.parseInt(dataList.get(i).get(1)));
          ps.setString(3, dataList.get(i).get(2));
          ps.setString(4, dataList.get(i).get(3));
        ...
        //displays times when final tee time = 1
        dbQuery = "SELECT playTimeId, ghin, lastName, firstName, teeTimeIndex, teeTime FROM " + tableName 
            + " WHERE finalTeeTime = 1";
        dataList = objDb.getData(displayColumnNames, dbQuery);
        //transfers the data from an arraylist to a 2d array
        data = objDb.to2dArray(dataList);
        //adds data, table, scroll pane to frame
        this.dbTable = new JTable(data, displayColumnNames);
        dbTable.setBackground(Welcome.ROSTER_COLOR);
        header = dbTable.getTableHeader();
        header.setBackground(Welcome.BUTTON_PANEL_COLOR);
        header.setFont(new Font("Trebuchet MS", Font.PLAIN, 16));
